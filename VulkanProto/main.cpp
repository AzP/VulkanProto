...
{
  const auto buf_info = vk::BufferCreateInfo().setUsage(vk::BufferUsageFlagBits::eUniformBuffer).setSize(size);
...
int setupPipelineStates()
{
  ...
  auto dynamicStateCreateInfo =
      vk::PipelineDynamicStateCreateInfo().setDynamicStateCount(static_cast<uint32_t>(dynamicStates.size())).setPDynamicStates(dynamicStates.data());
...
std::pair<vk::Semaphore, vk::Fence> recordCommandBuffer()
{
  ...
  cmd.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, g_info.pipelineLayout, 0, static_cast<uint32_t>(g_info.descriptorSets.size()),
                         g_info.descriptorSets.data(), 0, nullptr);
